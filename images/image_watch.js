 ...
 if(existLikeImage)
 {
   ...
   static loadImage=async(getImage,userimage)=>
  {
   ...
   let { urlimage, likes,description, DateTimePublish, title,user,idphoto,visibility} = getImage;
       ...
       document.getElementById("imagewatch_likesimage").innerHTML=likes;
   //CONERT FORMAT DATE
  ...
  static loadCommentImage=async(listcommentimage,idimage,iduser,userrname)=>
  {
    let html_comments_image="";
      ...
      for (let i = 0; i < listcommentimage.length; i++) {
        ...
        let textcomment=commentimage.textcomment ;
        let likescomment =commentimage.likescomment;
      ...
      // const loadSubComment= await  this.loadSubComment(listsubcomment,idcomment, iduser,userrname);
     ...
     if (existLikeImage) {
      const deletelikeImage= await APIRESTLikes.deletelikeImage(idimagewatch,
       ...
       if (deletelikeImage) {
         ...
         //ADD LIKE HTML
         let textcontent_numberlikes= document.getElementById('imagewatch_numberlikes').textContent;
         let numberoflikes=Number(textcontent_numberlikes)-1;
         document.getElementById('imagewatch_numberlikes').innerHTML=numberoflikes;
       }
      } 
     else{
...
imagewatch_buttonlikeimage.addEventListener('click', ImageWatchJS.like_dislike_Video);
